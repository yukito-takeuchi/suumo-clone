...
async function seed() {
  ...
  try {
    ...
    const tokyoId = prefectures.find(p => p.name === '東京都')?.id;
    const kanagawaId = prefectures.find(p => p.name === '神奈川県')?.id;
    ...
    const yamanoteLine = railwayLines.find(r => r.name === 'JR山手線')?.id;
    if (yamanoteLine) {
      await client.query(`
    ...
    const chuoLine = railwayLines.find(r => r.name === 'JR中央線')?.id;
    if (chuoLine) {
      await client.query(`
        INSERT INTO stations (railway_line_id, name) VALUES
